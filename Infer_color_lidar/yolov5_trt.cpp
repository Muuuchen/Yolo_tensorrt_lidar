    ...
    while (1)
    {
        ...
        for (int b = 0; b < fcount; b++)
        {
            ...
            for (size_t j = 0; j < res.size(); j++)
            {
                ...
                int cur_dis = (position[0]-select_point[0])*(position[0]-select_point[0]) +(position[1]-select_point[1])*(position[1]-select_point[1]);
                if(cur_dis < min_dis )
                {
                    min_dis = cur_dis;
                    send_index = j;
                ...
                std::string anchor_index = to_string(j);
                std::string _space = "  ";
                std::string label = my_classes[(int)res[j].class_id] + anchor_index;
                cv::putText(img, label, cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 3);
            }
        }
        select_point[0] = send_point[0];
        select_point[1] = send_point[1];
