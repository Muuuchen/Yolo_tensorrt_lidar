    ...
    while True:
        try:
            ...
            # Check if data contains buffer_end
                ...
                if image_buffer != b'':
                    image_buffer = np.frombuffer(image_buffer, dtype='uint8')
                    image_buffer = cv2.imdecode(image_buffer, cv2.IMREAD_COLOR)
                    if np.shape(image_buffer) == ():
                        reading = True
                        image_buffer = b''
                        continue

                    image_buffer = cv2.resize(image_buffer, (640, 480), interpolation = cv2.INTER_AREA)
