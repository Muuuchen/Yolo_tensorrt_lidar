...
int main(int argc, char **argv)
{
    ...
    std::string engine_name = "/home/robocon/Desktop/22th_Vision/Infer/best.engine";
    bool is_p6 = false;
    ...
    while (1)
    {
        ...
        for (int b = 0; b < fcount; b++)
        {
            ...
            // cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]);
          Anchor_data*  select= new Anchor_data[res.size] ;
          for (size_t j = 0; j < res.size(); j++)
            {
                ...
                //
                select[i].center = {r.x + r.width / 2, r.y + r.height / 2};
                
                float _depth = img_depth.at<float>(position[0], position[1]); // ?
                _depth = depth.get_distance(position[0], position[1]);
                float coor[3] = {0, 0, 0};
                get_3d_camera_coordinate(position, coor, _depth, &color_intrin, &depth_intrin, &depth2color);

                std::string _space = "   ";

                std::string label = my_classes[(int)res[j].class_id] + _space + std::to_string(coor[0]) + _space + std::to_string(coor[1]) + _space + std::to_string((coor[2])) + _space + std::to_string(_depth);
                ...
                cv::putText(img, jetson_fps, cv::Point(11, 80), cv::FONT_HERSHEY_PLAIN, 3, cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
                cv::Point p;
                p.x = position[0];
                p.y = position[1];
                              ...
                              rs2_intrinsics *color_intrin, rs2_intrinsics *depth_intrin, rs2_extrinsics *depth2color)
{
    ...
    float depth_point[3];
    float other_point[3];
    rs2_deproject_pixel_to_point(depth_pixel, depth_intrin, depth_pixel, depth);
                                ...
                                rs2::depth_frame *depth, float coor[3])
{
    ...
    int finalpos[2] = {centerx, centery};
    float finaldepth = center_depth;
